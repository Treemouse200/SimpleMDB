...
public class DefaultActorService : IActorService
{
    ...
    public async Task<Result<Actor>> UpdateActor(int id, Actor newActorData)
    {
        var validationResult = ValidateActor(newActorData);
        if (validationResult != null) { return validationResult; }
        var actor = await actorRepository.UpdateActor(id, newActorData);
        var result = actor == null
        ? new Result<Actor>(
         new Exception($"Could not update actor {newActorData} with id {id}."),
         (int)HttpStatusCode.NotFound)
        : new Result<Actor>(actor, (int)HttpStatusCode.OK);
        return result;
    }
